...
func (service serviceSend) wrapSendMessage(ctx context.Context, sessionID string, recipient types.JID, msg *waE2E.Message, content string) (whatsmeow.SendResponse, error) {
	client, err := whatsapp.GetClientOrDefault(sessionID)
	if err != nil {
		return whatsmeow.SendResponse{}, fmt.Errorf("failed to get WhatsApp client: %w", err)
	}
	...
	go func() {
		...
		// Set session ID in context for chat storage
		storeCtx = domainChatStorage.WithSessionID(storeCtx, sessionID)
...
func (service serviceSend) SendText(ctx context.Context, request domainSend.MessageRequest) (response domainSend.GenericResponse, err error) {
	...
	client, err := whatsapp.GetClientOrDefault(request.BaseRequest.SessionID)
	if err != nil {
		return response, fmt.Errorf("failed to get WhatsApp client: %w", err)
	}
	...
	ts, err := service.wrapSendMessage(ctx, request.BaseRequest.SessionID, dataWaRecipient, msg, request.Message)
	if err != nil {
		return response, err
	}
...
func (service serviceSend) SendImage(ctx context.Context, request domainSend.ImageRequest) (response domainSend.GenericResponse, err error) {
	...
	ts, err := service.wrapSendMessage(ctx, request.BaseRequest.SessionID, dataWaRecipient, msg, caption)
	go func() {
...
func (service serviceSend) SendContact(ctx context.Context, request domainSend.ContactRequest) (response domainSend.GenericResponse, err error) {
	...
	ts, err := service.wrapSendMessage(ctx, request.BaseRequest.SessionID, dataWaRecipient, msg, content)
	if err != nil {
		return response, err
	}
