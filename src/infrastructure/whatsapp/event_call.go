...
func handleCallOffer(ctx context.Context, evt *events.CallOffer, deviceID string, client *whatsmeow.Client) {
	...
	if len(config.WhatsappWebhook) > 0 {
		go func(e *events.CallOffer, c *whatsmeow.Client) {
			webhookCtx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
			defer cancel()
			if err := forwardCallOfferToWebhook(webhookCtx, e, deviceID, c); err != nil {
...
func createCallOfferPayload(ctx context.Context, evt *events.CallOffer, deviceID string, client *whatsmeow.Client) map[string]any {
	...
	payload["auto_rejected"] = config.WhatsappAutoRejectCall
...
func forwardCallOfferToWebhook(ctx context.Context, evt *events.CallOffer, deviceID string, client *whatsmeow.Client) error {
	payload := createCallOfferPayload(ctx, evt, deviceID, client)
