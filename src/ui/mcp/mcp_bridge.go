...
func (b *AIBridge) toolProcessMessage() mcp.Tool {
	return mcp.NewTool("ai_process_message",
		...
		mcp.WithString("message_type", mcp.DefaultString("text"), mcp.Description("Type of message (e.g., text, image)")),
...
func (b *AIBridge) handleProcessMessage(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	return mcp.NewToolResultStructured(result, "AI response processed successfully"), nil
}
...
func (b *AIBridge) SendAIRequest(ctx context.Context, request AIRequest) (*ProcessMessageResponse, error) {
	...
	// Debug logging
	b.logger.WithField("request_json", string(jsonData)).Info("Sending AI request")
	...
	b.logger.WithField("url", url).Info("Making HTTP request to AI service")
	...
	b.logger.WithField("status_code", resp.StatusCode).Info("AI service response status")
	...
	b.logger.WithField("response", response).Info("AI service response decoded successfully")
