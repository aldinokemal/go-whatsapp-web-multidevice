...
| `WHATSAPP_FULL_HISTORY_SYNC`            | Request a full message-history sync from the phone. **Applies only when a device links (fresh QR / re-pair) — toggling it on an already-paired device does nothing until the next pairing.** Off = the recent window only. | `false` | `WHATSAPP_FULL_HISTORY_SYNC=true`             |
| `WHATSAPP_FULL_HISTORY_SYNC_DAYS_LIMIT` | Days of history to request when `WHATSAPP_FULL_HISTORY_SYNC=true`. `0` = whatsmeow's default. A full sync is a large payload processed in the event handler and grows the chat store, so **set a bound (e.g. 90) on large/active accounts** rather than leaving it unlimited. | `0` | `WHATSAPP_FULL_HISTORY_SYNC_DAYS_LIMIT=90`    |
