...
A history sync only reopens a resolved conversation *after* a message has actually been posted into it, so a run with nothing new to add leaves the thread alone. Before posting into a resolved conversation, the sync first writes a durable reopen intent to the same retry queue the live forwards use, keyed by conversation — that write, not the reopen call itself, is what makes the whole sequence recoverable: if it fails, nothing has been posted or linked yet, so the sync aborts and the next run simply retries from scratch with no duplicate risk. The pre-armed intent records the message IDs being posted, making it inert by construction: the retry worker only replays it if at least one of those messages is confirmed linked in local storage. If the run then goes on to post but the live reopen call fails (or the queue write refreshing it also fails), the pre-armed row is what's left to finish the job — it is replayed in the background until the thread is open again, and the queued row holds only the conversation, so the retry never reposts a message. A pass where nothing ends up posted (every message rejected, media that never lands) voids its pre-armed row, and even if that cancellation write fails the worker safely drops the unconfirmed intent so a thread nothing was added to never gets reopened. A row is otherwise dropped once the conversation is no longer resolved, when the thread has newer activity than the queued intent (an agent resolved it again after the fact, so that decision stands), when the device stops pointing at that Chatwoot account, when `CHATWOOT_REOPEN_CONVERSATION` has been set to `false` since the intent was queued, or 24 hours after it was queued.
