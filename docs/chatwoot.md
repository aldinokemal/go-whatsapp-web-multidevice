...
A history sync only reopens a resolved conversation *after* a message has actually been posted into it, so a run with nothing new to add leaves the thread alone. If the reopen call itself fails after the message landed, the intent is written to the same retry queue the live forwards use, keyed by conversation, and replayed in the background until the thread is open again — the queued row holds only the conversation, so the retry never reposts a message. It is dropped once the conversation is no longer resolved, when the thread has newer activity than the queued intent (an agent resolved it again after the fact, so that decision stands), when the device stops pointing at that Chatwoot account, when `CHATWOOT_REOPEN_CONVERSATION` has been set to `false` since the intent was queued, or 24 hours after it was queued.
